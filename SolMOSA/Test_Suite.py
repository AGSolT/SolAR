...
class TestSuite():
    ...
    def __init__(self, _SmartContract, _accounts, _deploying_accounts, _pop_size=50, _random=True, _tests=[], _max_method_calls=10,
                 _min_method_calls=0, _passBlocks=False, _passTime=False, _passTimeTime=None, _maxWei=10000000000000000000):
        """
        ...
        if not _random:
            ...
            for i in range(_pop_size):
                tests = tests + [TestCase(_methodCalls=None, _random=True, SmartContract=_SmartContract, accounts=_accounts,
                                          deploying_accounts=_deploying_accounts, max_method_calls=_max_method_calls,
                                          min_method_calls=_min_method_calls, passBlocks=_passBlocks, passTime=_passTime,
    ...
    def show_tests(self, _top_n=None, log=False):
        ...
        if log:
            ...
            if _top_n is None:
                ...
                assert _top_n <= len(
                    self.tests), "The number of tests that can be shown cannot exceed the total number of tests!"
